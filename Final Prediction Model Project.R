...
file_path <- "CAMpaper_fulldata.xlsx"
...
#Encoding does not need to be done separately (train and test)
...
# Split data before certain pre-processing:
...
process_numerical_data <- function(data) { # create a function with the name my_function
  ...
  #outlier detection and replacement for numerical data
...
scale_numerical_data <- function(data) {
...
training_data <- process_numerical_data(training_data)
testing_data <- process_numerical_data(testing_data)

training_data <- scale_numerical_data(training_data)
testing_data <- scale_numerical_data(testing_data)
...
smoteData <- SMOTE(X = subset(training_data, select = -Asthma_ER_or_Hospitalization), target = training_data$Asthma_ER_or_Hospitalization, K = 5, dup_size = 2)

interim_data <- smoteData$data
